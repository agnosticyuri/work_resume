...
def load_las_data(file_path, use_extra_features=False):
    ...
    labels = las.classification  # Extract classification labels 0......42
...
if __name__ == "__main__":
    ...
    if use_extra_features:
        input_dim = train_points.shape[1] + train_features.shape[1]  # Combine points and features
        train_data = np.hstack((train_points, train_features))
    else:
        input_dim = train_points.shape[1]  # Only use coordinates
        train_data = train_points

    num_classes = len(np.unique(train_labels))
    model = PointCNN(input_dim, num_classes)

    # Train model with checkpointing
    train_model(model, train_data, train_labels, checkpoint_path=checkpoint_path)
    ...
    new_points, _ = load_las_data("C:/fastload/hrad50.las", use_extra_features=False)
